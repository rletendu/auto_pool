...
const char HTTP_PORTAL_OPTIONS[] PROGMEM =
    ...
    "method=\"get\"><button>Configure WiFi (No Scan)</button></form><br/>";
//<form action=\"/i\" method=\"get\"><button>Info</button></form><br/><form
...
const char HTTP_SAVED[] PROGMEM =
    "<div>Credentials Saved<br />Trying to connect Weread to network.<br />If "
    "it fails reconnect to AP to try again</div>";
...
class WiFiManager {
 public:
  ...
  // in a webserver loop in seconds setConfigPortalTimeout is a new name for
  // setTimeout
  void setConfigPortalTimeout(unsigned long seconds);
  void setTimeout(unsigned long seconds);

  // sets timeout for which to attempt connecting, useful if you get a lot of
  // failed connects
  ...
  void setProgressCallback(void (*func)(WiFiManager *myWiFiManager, uint32_t call_count), uint8_t period);
  ...
  // if this is set, try WPS setup when starting (this will delay config portal
  // for up to 2 mins)
  // TODO
 ...
 private:
  ...
  void (*_progressscallback)(WiFiManager *, uint32_t) = NULL;
  uint32_t _period_callback=0;
  ...
  auto optionalIPFromString(...) -> bool {
    DEBUG_WM(
        "NO fromString METHOD ON IPAddress, you need ESP8266 core 2.1.0 or "
        "newer for Custom IP configuration to work.");
    return false;
  }
};
