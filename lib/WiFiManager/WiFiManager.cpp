...
void WiFiManager::addParameter(WiFiManagerParameter *p)
{
  if (_paramsCount + 1 > WIFI_MANAGER_MAX_PARAMS)
  {
    ...
    DEBUG_WM(
        "WIFI_MANAGER_MAX_PARAMS exceeded, increase number (in WiFiManager.h) "
...
void WiFiManager::setupConfigPortal()
{
  ...
  server->on("/fwlink",
             std::bind(&WiFiManager::handleRoot,
                       this)); // Microsoft captive portal. Maybe not needed.
...
{
  unsigned long time_last_progress_call = millis();
  // setup AP
  ...
  while (1)
  {
    ...
    if (_progressscallback != NULL && _period_callback)
    {
      if (millis() > (time_last_progress_call + _period_callback))
      {
        DEBUG_WM(F("Call progress CB"));
        _progressscallback(this, ++progress_count);
        time_last_progress_call = millis();
      }
    }
    // check if timeout
...
void WiFiManager::handleRoot()
{
  ...
  if (captivePortal())
  { // If caprive portal redirect instead of displaying
    // the page.
    return;
  }
...
{
  if (captivePortal())
  { // If captive portal redirect instead of displaying
    // the error page.
    return;
  }
...
// Progress call_back
void WiFiManager::setProgressCallback(void (*func)(WiFiManager *myWiFiManager, uint32_t call_count), uint8_t second)
{
  _progressscallback = func;
  _period_callback = 1000*second;
}
