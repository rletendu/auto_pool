
#ifndef __CONFIG_H__
#define __CONFIG_H__

#define AUTOPOOL_VER 2.1
#define PORTAL_NAME "AUTOPOOL_CONFIG"
#define PORTAL_CONFIG_TIMEOUT_S 60
#define PORTAL_CONFIG_PROGRESS_S 5

#define MQTT_CLIENT_NAME "AUTOPOOL"
#define HOST_NAME "AUTOPOOL"
#define NTPSERVER "pool.ntp.org"
#define GMTOFFSET 3600
#define DAYLIGHTOFFSET 3600

#define PARAMETER_FILENAME "/config.json"
#define STATE_FILENAME "/state.json"
#define PARAMETERS_JSON_MESSAGE_LEN 768
#define MEASURES_JSON_MESSAGE_LEN 512
#define STATE_FILTER_CTRL_JSON_MESSAGE_LEN 300
#define STATE_ORP_CTRL_JSON_MESSAGE_LEN 300
#define STATE_PH_CTRL_JSON_MESSAGE_LEN 300

#define MEASURES_UPDATE_S 4
#define MQTT_HEARTBEAT_S 300
#define MQTT_MIN_INTERVAL_S 120
#define PH_MIN_DELTA       0.05f
#define ORP_MIN_DELTA      5.0f
#define PRESSURE_MIN_DELTA 0.05f
#define SYS_TEMP_MIN_DELTA     0.3f
#define SYS_HUMIDITY_MIN_DELTA 1.0f
#define MEASURE_QUIET_MODE_UPDATE_S 30 * 60
#define GRAPH_UPDATE_S 5
#define FILTER_CONTROL_UPDATE_S 10
#define FILTER_CONTROL_WARM_UP_S 30
#define FILTER_CONTROL_PERIODIC_DURATION_S 60
#define FILTER_PUMP_ON_MIN_TIME_S 30
#define ORP_CONTROL_UPDATE_S 10
#define ORP_REGULATION_CYCLE_S 10 * 60
#define PH_CONTROL_UPDATE_S 10
#define PH_REGULATION_CYCLE_S 10 * 60

#define BOOT_KEY_MAGIC 0x55AA1234

//#define DEBUG_PIN1 PIN_EXT1

#define HAS_PH_PLUS_PUMP 0
#define HAS_QUIET_MEASURES 0
#define HAS_FILTER_CONTROL 1
#define HAS_MEASURE_CONTROL 1
#define HAS_ORP_CONTROL 1
#define HAS_PH_CONTROL 1
#define HAS_FILTER_PWR_CTRL 1

#define HAS_CLI 1
#define HAS_OTA 1
#define HAS_WEB_SERVER 1
#define HAS_MQTT 1
#define HAS_TELNET_SERVER 0

#define DISPLAY_BAUD 115200
#define DISP_LOG_NB_LINES 12
#define DISP_LOG_NB_VISIBLE_LINES DISP_LOG_NB_LINES
#define DISP_LOG_NB_CHAR_PER_LINE 42

#define NB_BOOT_STEP_MSG 15
#define GRAPH_MAX_PTS 150

#define TOUCH_TIMEOUT_S 60

#define HAS_CUSTOM_MAC 0
#define CUSTOM_MAC {0x24,0x6F,0x28,0xA1,0x5E,0x34}

#endif